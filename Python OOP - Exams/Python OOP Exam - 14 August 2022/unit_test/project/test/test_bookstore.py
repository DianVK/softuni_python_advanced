...
class Test(TestCase):
    ...
    def test_books_limit_zero_raise_value_error(self):
    ...
    def test_correct_len_method(self):
        ...
        self.assertEqual(13, self.store.__len__())

    def test_not_enough_spaces_to_receive_books_raise_exception(self):
        self.store.availability_in_store_by_book_titles = self.books
    ...
    def test_add_existing_book_correct(self):
        ...
        self.assertEqual("4 copies of C++ are available in the bookstore.", result)
    ...
    def test_sell_more_copies_than_available_raises_exception(self):
        self.store.availability_in_store_by_book_titles = self.books
        with self.assertRaises(Exception) as ex:
            self.store.sell_book("Java", 11)

        self.assertEqual("Java has not enough copies to sell. Left: 10", str(ex.exception))

    def test_successful_selling_a_book(self):
        self.store.availability_in_store_by_book_titles = self.books
