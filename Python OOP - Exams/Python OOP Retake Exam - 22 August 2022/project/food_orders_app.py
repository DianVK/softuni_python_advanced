...
class FoodOrdersApp:
    ...
    def add_meals_to_shopping_cart(self, client_phone_number: str, **meal_names_and_quantities):
        ...
        current_menu = self.__get_menu()
        Validation.meal_in_menu(meal_names_and_quantities, current_menu)
        Validation.enough_quantity(meal_names_and_quantities, current_menu)

        for name, quantity in meal_names_and_quantities.items():
            current_menu[name].quantity -= quantity
            find_client.shopping_cart.append(current_menu[name])

        find_client.bill += sum(current_menu[name].price * qty for name, qty in meal_names_and_quantities.items())
